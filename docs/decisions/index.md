...
| | |
|---|---|
| [ADR-0001](ADR-0001-requirements-are-read-out-of-the-code.md) | the requirements are read out of the code, not written before it |
| [ADR-0002](ADR-0002-the-policy-engine-is-real-in-tests.md) | the OPA policy engine is real in tests, and the suite refuses to start without it |
| [ADR-0003](ADR-0003-every-other-boundary-is-faked-at-the-sdk-client.md) | every other external boundary is faked at the `celine-sdk` client |
| [ADR-0004](ADR-0004-the-database-is-real-and-sqlite.md) | the database is real in tests, and it is SQLite |
